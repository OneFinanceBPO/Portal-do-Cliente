...
export default async function LogPage({ searchParams }: { searchParams: { categoria?: string } }) {
  ...
  return (
    <main className="page">
      ...
      <div className="filter-row">
        <a href="/log" className={`tog ${!categoria ? 'on' : ''}`} style={{ display: 'inline-block', textDecoration: 'none' }}>Todas</a>
        {CATEGORIAS.map((c) => (
      ...
      <div className="tbl-wrap log-table">
        <table>
          <thead>
            <tr><th>Data/hora</th><th>Usuário</th><th>Categoria</th><th>Ação</th><th>Detalhe</th></tr>
          </thead>
          <tbody>
            {logs.map((log) => (
              <tr key={log.id}>
            ...
            {logs.length === 0 && <tr><td colSpan={5}>Nenhum registro encontrado.</td></tr>}
          </tbody>
        </table>
      </div>
    </main>
